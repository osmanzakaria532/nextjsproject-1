...
export default function SideBar({ docs }) {
  ...
  useEffect(() => {
    ...
    if (pathName.includes("/tags")) {
      ...
      matchedDoc = getDocumentByTags(docs, tag);
    } else if (pathName.includes("/authors")) {
      const author = pathName.split("/")[2];
      matchedDoc = getDocumentByAuthor(docs, author);
    } else if (pathName.includes("/categories")) {
      const category = pathName.split("/")[2];
      matchedDoc = getDocumentByCategory(docs, category);
    }

    const roots = matchedDoc.filter((doc) => !doc.parent);
    // console.log(roots);

    const nonRoots = Object.groupBy(
      matchedDoc.filter((doc) => doc.parent),
      ({ parent }) => parent
    );
